<?php
	 ...
	 class Jefferson_Provador_Block_Product_View_Options_Type_Select extends Mage_Catalog_Block_Product_View_Options_Type_Select {
            ...
            foreach ($_option->getValues() as $_value) {
                ...
                if ($arraySign) {
					...
					if($type == 'checkbox'){
					       ...
					       $provador = Mage::helper('provador')->getSkuCustomOption($_option->getOptionId());

    					    $selectHtml .= '<li class="'.$provador.'"><div class="provador '.$provador.'">' . '<input type="' . $type . '" class="' . $class  . ' ' . $require
    					    . ' product-custom-option '.$provador.'"'
    					        . ($this->getSkipJsReloadPrice() ? '' : ' onclick="opConfig.reloadPrice()"')
